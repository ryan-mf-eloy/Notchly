...
`train.py` and `compare_baselines.py` calibrate with `--min-threshold` (`0.50` by default) so bootstrap promotion is precision-first. Candidate checkpoints can use `audio_encoder=temporal_cnn`: a separate convolutional encoder over 40-band audio/proxy frames, matching the MultiQT-style audio/text encoder split more closely than the legacy `summary_stats` encoder. The bundled checkpoint remains `summary_stats`/`signal_proxy` because a 2026-05-28 `temporal_cnn` candidate passed aggregate gates but failed the targeted Core ML runtime smoke for `Quais sao os principios SOLID de programacao`. Baseline promotion requires absolute precision/recall/latency/zero-critical-FP gates, targeted qualitative smokes, and a text-only comparison win through precision, critical-FP reduction, or recall gain while preserving the absolute precision and zero-critical-FP gates, instead of allowing a low dev threshold that leaks critical negatives on the hard split.
