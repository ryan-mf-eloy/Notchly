...
struct LanguageContinuityResolver {
    ...
    mutating func resolve(
        ...
        guard let detection = detectedLanguage(for: text, isFinal: isFinal),
              let detected = SupportedLanguage.language(for: detection.languageCode) else {
            states[audioSource] = State(language: fallback, confidence: 0.45, updatedAt: Date())
            return (fallback, false, 0.45)
        }

        let previous = states[audioSource]?.language ?? SupportedLanguage.language(for: existingLanguage)
        let shouldSwitch: Bool
        if previous == nil || previous == detected {
            shouldSwitch = true
        } else if isFinal {
            shouldSwitch = detection.confidence >= 0.58
        } else {
            shouldSwitch = detection.confidence >= 0.82 && wordCount(in: text) >= 5
        }

        let language = shouldSwitch ? detected : (previous ?? fallback)
        states[audioSource] = State(language: language, confidence: detection.confidence, updatedAt: Date())
        return (language, shouldSwitch, detection.confidence)
    }
